  ...
  return (
    ...
    >
      ...
      <div className="animate-float-gentle">
        <span
          className="block text-7xl animate-pulse-glow"
          style={{ filter: "drop-shadow(0 0 24px rgba(196,151,63,0.5))" }}
        >
          🪩
        </span>
      </div>
      ...
      <div>
        <p className="font-display italic text-2xl sm:text-3xl mb-2" style={{ color: "#4A3728" }}>
...
export default function CheckoutPage() {
  ...
  // Mostra tela de redirect ANTES de checar carrinho vazio
    ...
    <main className="min-h-screen pt-8 pb-16 px-4" style={{ background: "#F4ECD8" }}>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-10">
          <Link href="/produtos" className="text-2xl" aria-label="Voltar">←</Link>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            <section className="p-6 rounded-3xl" style={{ background: "#fff9f2", border: "1px solid #E5D5B5" }}>
              <h2 className="font-display italic text-2xl mb-5" style={{ color: "#4A3728" }}>Seus dados</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>Nome completo *</label>
                  <input className="input-base" type="text" required placeholder="Maria da Silva"
                    value={form.nome} onChange={set("nome")} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>E-mail *</label>
                    <input className="input-base" type="email" required placeholder="maria@email.com"
                      value={form.email} onChange={set("email")} />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>Telefone</label>
                    <input className="input-base" type="tel" placeholder="(11) 99999-9999"
            ...
            <section className="p-6 rounded-3xl" style={{ background: "#fff9f2", border: "1px solid #E5D5B5" }}>
              <h2 className="font-display italic text-2xl mb-5" style={{ color: "#4A3728" }}>Endereço de entrega</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>CEP *</label>
                  <input className="input-base" type="text" required placeholder="00000-000"
                    maxLength={9} value={form.cep} onChange={set("cep")} />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>Rua, número, complemento *</label>
                  <input className="input-base" type="text" required placeholder="Rua das Flores, 42, apto 3"
                    value={form.endereco} onChange={set("endereco")} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>Cidade *</label>
                    <input className="input-base" type="text" required placeholder="São Paulo"
                      value={form.cidade} onChange={set("cidade")} />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: "#4A3728" }}>UF *</label>
                    <select className="input-base" required value={form.uf} onChange={set("uf")}>
                      <option value="">—</option>
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-8 p-6 rounded-3xl space-y-4"
              style={{ background: "#fff9f2", border: "1px solid #E5D5B5" }}>
              <h2 className="font-display italic text-2xl" style={{ color: "#4A3728" }}>Resumo</h2>
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {item.tamanho && <p className="text-xs" style={{ color: "#B8A898" }}>Tam. {item.tamanho}</p>}
                    </div>
              ...
              <div className="border-t pt-4 flex justify-between items-center" style={{ borderColor: "#E5D5B5" }}>
